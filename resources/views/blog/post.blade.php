<x-layouts.layout>
    ...
    @php
        ...
        $avatar = ((array) $author->avatar_urls)['96'];

        $pubDate = \Carbon\Carbon::createFromDate($post->modified);

        $replies = $post->_embedded->replies;
    @endphp
                ...
                <header class="mb-4 lg:mb-6 not-format">
                    <address class="flex items-center mb-6 not-italic">
                        <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                            <img class="mr-4 w-16 h-16 rounded-full" src="{{ $avatar }}" alt="{{ $author->name }}">
                            <div>
                                {{-- <a href="#" rel="author" class="text-xl font-bold text-gray-900 dark:text-white">{{ $author->name }}</a> --}}
                                <span rel="author"
                                    class="text-xl font-bold text-gray-900 dark:text-white">{{ $author->name }}</span>
                                <p class="text-base font-light text-gray-500 dark:text-gray-400">
                                    {{ $author->description }}</p>
                                <p class="text-base font-light text-gray-500 dark:text-gray-400"><time pubdate
                                        datetime="{{ $pubDate->format('Y-m-d') }}"
                                        title="{{ $pubDate->format('m-d-y') }}">{{ $pubDate->format('M. d, Y') }}</time>
                                </p>
                            </div>
                        </div>
                    </address>
